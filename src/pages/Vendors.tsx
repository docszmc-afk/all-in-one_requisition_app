...
export default function Vendors() {
  ...
  const getVendorHistory = (vendorId: string, vendorName: string) => {
    return requests.filter(req => 
      req.status === 'Approved' && 
      (
        req.splitSupplier === vendorName || 
        (req.items && req.items.some(item => item.supplier === vendorName)) ||
        req.dieselDetails?.vendorId === vendorId ||
        (req.dieselDetails?.vendorId === 'manual' && req.dieselDetails?.manualVendorName === vendorName) ||
        req.productProcurementDetails?.vendorId === vendorId ||
        (req.productProcurementDetails?.vendorId === 'manual' && req.productProcurementDetails?.manualVendorName === vendorName)
      )
    );
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/80 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <div className="p-6 overflow-y-auto flex-1">
              {getVendorHistory(selectedVendor.id, selectedVendor.name).length > 0 ? (
                <div className="space-y-4">
                  {getVendorHistory(selectedVendor.id, selectedVendor.name).map(req => (
                    <div key={req.id} className="bg-stone-50 border border-stone-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      ...
                      <div className="text-right">
                        ...
                        <a href={`/requests/${req.id}`} className="text-sm font-medium text-orange-600 hover:text-orange-700 mt-1 inline-block">
                          View Details &rarr;
                        </a>
                      </div>
                    </div>
