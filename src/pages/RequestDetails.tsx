...
export default function RequestDetails() {
  ...
  const isPdf = previewAttachment && (
    (previewAttachment.type && typeof previewAttachment.type === 'string' && previewAttachment.type.toLowerCase().includes('pdf')) || 
    (previewAttachment.name && typeof previewAttachment.name === 'string' && previewAttachment.name.toLowerCase().endsWith('.pdf'))
  );
  ...
  const [isRequestBought, setIsRequestBought] = useState(false);
  const [isRequestInspected, setIsRequestInspected] = useState(false);
  ...
  React.useEffect(() => {
    if (request) {
      ...
      if (request.histologyDetailsList && request.histologyDetailsList.length > 0) {
        ...
        setEditableHistologyDetails([]);
      }
      
      setRecommendedAmount(request.recommendedAmount ?? '');
      setIsRequestBought(request.isBought || false);
      setIsRequestInspected(request.isInspected || false);
      
      // Reset state that might carry over from previous request
      setPreviewAttachment(null);
      setNotes('');
      setApproverSignature('');
      
      // Scroll to top when viewing a new request
      window.scrollTo(0, 0);
    }
  }, [request?.id]);
      ...
      const similar = requests.filter(r => 
        ...
        r.items?.some(ri => 
          request.items?.some(i => 
            (i && ri && i.name && ri.name && typeof i.name === 'string' && typeof ri.name === 'string' && ri.name.toLowerCase().includes(i.name.toLowerCase())) ||
            (i && ri && i.description && ri.description && typeof i.description === 'string' && typeof ri.description === 'string' && ri.description.toLowerCase().includes(i.description.toLowerCase()))
          )
        )
      );
  ...
  const handleVoid = async () => {
    ...
    try {
      if (request && request.workflow && request.currentApproverIndex !== undefined) {
        ...
        // Void the approver
        await voidApprover(request.id, currentApproverId, voidReason);
        ...
        addNotification({
          userId: user?.id || 'system',
          title: 'Success',
          message: 'Approver has been voided successfully.',
          type: 'success'
        });
      } else {
        throw new Error('This request does not have a valid approval workflow to void.');
      }
    } catch (error: any) {
      console.error('Error voiding approver:', error);
      addNotification({
        userId: user?.id || 'system',
        title: 'Error',
        message: error.message || 'Failed to void approver. Please try again.',
        type: 'error'
      });
    } finally {
      setIsVoiding(false);
  ...
  const canVoid = isDynamicWorkflow && request.status === 'Pending Approval' && (
  ...
  const handleSaveInspection = async () => {
    ...
    try {
      await updateRequest(request.id, {
        items: editableItems,
        isBought: isRequestBought,
        isInspected: isRequestInspected,
        inspectedBy: user?.id,
      ...
      addNotification({
        ...
        message: 'Inspection details have been updated successfully.',
        type: 'success'
      });
    } catch (error) {
      ...
      addNotification({
        userId: user?.id || 'system',
        title: 'Error',
        message: 'Failed to save inspection details.',
        type: 'error'
      });
    } finally {
  ...
  const handleItemChange = (index: number, field: keyof ProcurementItem, value: any) => {
    ...
    // Recalculate recommendedAmount if audit is editing and the field is a price field
    if (isAuditEditing && (field === 'audit1RecommendedPrice' || field === 'audit2RecommendedPrice')) {
      let total = 0;
      if (isAudit2) {
        total = newItems.reduce((sum, item) => sum + (Number(item.audit2RecommendedPrice || 0) * (item.quantity || 0)), 0);
      } else if (isAudit1 || isGenericAudit) {
        total = newItems.reduce((sum, item) => sum + (Number(item.audit1RecommendedPrice || 0) * (item.quantity || 0)), 0);
      }
      setRecommendedAmount(total);
    }
  };
  ...
  const handleHistologyChange = (index: number, field: keyof HistologyDetails, value: any) => {
    ...
    if (isAuditEditing && (field === 'audit1RecommendedPrice' || field === 'audit2RecommendedPrice')) {
      let total = 0;
      if (isAudit2) {
        total = newDetails.reduce((sum, item) => sum + Number(item.audit2RecommendedPrice || 0), 0);
      } else if (isAudit1 || isGenericAudit) {
        total = newDetails.reduce((sum, item) => sum + Number(item.audit1RecommendedPrice || 0), 0);
      }
      setRecommendedAmount(total);
    }
  };
  ...
  const handleRunMarketComparer = async () => {
    ...
    try {
      ...
      addNotification({
        userId: user?.id || 'system',
        title: 'AI Error',
        message: 'Failed to run AI Market Comparer. Check your API key.',
        type: 'error',
      });
    } finally {
  ...
  const handleAction = async (action: 'Approve' | 'Reject' | 'Sent Back') => {
    if (!approverSignature) {
      addNotification({
        ...
        message: 'Please sign before submitting your decision.',
        type: 'warning',
      });
      return;
    }
  ...
  const safeFormat = (dateString: string | undefined | null, formatStr: string, fallback = 'Unknown Date') => {
    ...
    try {
      ...
      if (request.attachments && request.attachments.length > 0) {
        for (const att of request.attachments) {
          const isPdf = (att.type && typeof att.type === 'string' && att.type.includes('pdf')) || (att.name && typeof att.name === 'string' && att.name.toLowerCase().endsWith('.pdf'));
          const isImage = (att.type && typeof att.type === 'string' && att.type.includes('image')) || (att.name && typeof att.name === 'string' && att.name.toLowerCase().match(/\.(jpg|jpeg|png)$/));
          const isPng = (att.type && typeof att.type === 'string' && att.type.includes('png')) || (att.name && typeof att.name === 'string' && att.name.toLowerCase().endsWith('.png')) || (att.data && typeof att.data === 'string' && att.data.startsWith('data:image/png'));
    ...
    try {
      ...
      if (targetUserId) {
        ...
        addNotification({
          ...
          message: 'Could not determine the current approver.',
          type: 'error',
        });
      }
    } catch (error) {
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <div className="mt-3 text-center sm:mt-5">
                    ...
                    <div className="mt-2 text-left max-h-60 overflow-y-auto">
                      {similarRequests.filter(Boolean).map(req => (
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4 pb-4 border-b border-stone-100">
                ...
                {request.splitSupplier && (
                  <div>
                    <h3 className="text-sm font-medium text-stone-500">Supplier</h3>
                    <p className="mt-1 text-sm font-medium text-stone-900 flex items-center">
                      <Tag className="w-4 h-4 mr-1 text-stone-400" />
                      {request.splitSupplier}
                    </p>
              ...
              <div className="grid grid-cols-2 gap-4 pt-4 border-t border-stone-100">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm font-medium text-stone-900 flex items-center">
                    ...
                    {safeFormat(request.createdAt, 'MMM d, yyyy h:mm a')}
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-stone-200">
                  ...
                  <tbody className="bg-white divide-y divide-stone-200">
                    {editableHistologyDetails.filter(Boolean).map((detail, index) => (
                        ...
                        {(isAuditEditing || detail.audit1RecommendedPrice || detail.audit2RecommendedPrice || detail.flagged) && (
                          <>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-right">
                              {isAuditEditing ? (
                                <MoneyInput
                                  value={isAudit2 ? (detail.audit2RecommendedPrice || 0) : (detail.audit1RecommendedPrice || 0)}
                                  onChange={(val) => handleHistologyChange(index, isAudit2 ? 'audit2RecommendedPrice' : 'audit1RecommendedPrice', val)}
                                ...
                                <span className="text-stone-900 font-medium">
                                  {detail.audit2RecommendedPrice != null ? `₦${detail.audit2RecommendedPrice.toLocaleString()}` : detail.audit1RecommendedPrice != null ? `₦${detail.audit1RecommendedPrice.toLocaleString()}` : '-'}
                                </span>
                              )}
                            </td>
              ...
              <div className="p-8 border-b border-stone-100">
                ...
                <div className="flex justify-between items-center mb-8">
                  <div className="text-sm">
                    <span className="font-semibold text-stone-700">Date:</span> {safeFormat(request.paymentDetails.date, 'MMMM d, yyyy')}
                  </div>
                  <div className="text-sm">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-stone-200 overflow-hidden">
              ...
              <div className="p-6">
                <dl className="grid grid-cols-1 sm:grid-cols-3 gap-x-4 gap-y-6">
                  ...
                  {request.productProcurementDetails.vendorId && (
                    <div className="sm:col-span-3"><dt className="text-sm font-medium text-stone-500">Vendor</dt><dd className="mt-1 text-sm text-stone-900">
                      {getVendorName(request.productProcurementDetails.vendorId, request.productProcurementDetails.manualVendorName)}
                    </dd></div>
                  )}
                </dl>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-stone-200 overflow-hidden">
              ...
              <div className="p-6">
                <dl className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-6">
                  ...
                  {request.leaveDetails.leaveBalance !== undefined && (
                    <div><dt className="text-sm font-medium text-stone-500">Number of Leave Days Left</dt><dd className="mt-1 text-sm text-stone-900">{request.leaveDetails.leaveBalance}</dd></div>
                  )}
                  <div><dt className="text-sm font-medium text-stone-500">Purpose of Leave</dt><dd className="mt-1 text-sm text-stone-900">{request.leaveDetails.purpose}</dd></div>
                </dl>
              </div>
            </div>
          )}
                ...
                <div className="flex space-x-3">
                  {canInspect && (
                    <button
                      ...
                      className="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-lg shadow-sm text-white bg-emerald-600 hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 disabled:opacity-50"
                    >
                ...
                <table className="min-w-full divide-y divide-stone-200">
                  <thead className="bg-stone-50">
                    <tr>
                      ...
                      {(isStoreEditing || isAuditEditing || request.items.some(i => i && i.recommendedPrice)) && (
                        <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-stone-500 uppercase tracking-wider">Store Rec.</th>
                      )}
                      {(isAuditEditing || request.items.some(i => i && i.audit1RecommendedPrice)) && (
                        <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-stone-500 uppercase tracking-wider">Audit 1 Rec.</th>
                      )}
                      {(isAuditEditing || request.items.some(i => i && i.audit2RecommendedPrice)) && (
                        <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-stone-500 uppercase tracking-wider">Audit 2 Rec.</th>
                      )}
                      {(isStoreEditing || isAuditEditing || request.items.some(i => i && i.supplier)) && (
                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-stone-500 uppercase tracking-wider">Supplier</th>
                      )}
                      {(isStoreEditing || isAuditEditing || request.items.some(i => i && i.availability)) && (
                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-stone-500 uppercase tracking-wider">Availability</th>
                      )}
                      {(isAuditEditing || request.items.some(i => i && (i.flagged || i.aiMarketPrice))) && (
                      ...
                      {request.status === 'Approved' && isInspectionEligible && (
                        <>
                          <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-stone-500 uppercase tracking-wider">Bought</th>
                          <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-stone-500 uppercase tracking-wider">Inspected</th>
                        </>
                      )}
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-stone-200">
                    {(isStoreEditing || isAuditEditing || canInspect ? editableItems : request.items).filter(Boolean).map((item, index) => (
                      <tr key={item.id} className={item.flagged ? 'bg-red-50' : ''}>
                        ...
                        {(isStoreEditing || isAuditEditing || request.items.some(i => i && i.recommendedPrice)) && (
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-stone-500 text-right">
                            {isStoreEditing ? (
                              <MoneyInput
                                value={item.recommendedPrice || 0}
                                onChange={(val) => handleItemChange(index, 'recommendedPrice', val)}
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-stone-500 text-right">
                            {(isAudit1 || isGenericAudit) ? (
                              <div className="flex flex-col gap-2 items-end">
                                <MoneyInput
                                  value={item.audit1RecommendedPrice || 0}
                                  onChange={(val) => handleItemChange(index, 'audit1RecommendedPrice', val)}
                                  className="w-24 text-right rounded-md border-stone-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm"
                                  placeholder="Price"
                                />
                                <input
                        ...
                        {(isAuditEditing || request.items.some(i => i && i.audit2RecommendedPrice)) && (
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-stone-500 text-right">
                            {(isAudit2) ? (
                              <div className="flex flex-col gap-2 items-end">
                                <MoneyInput
                                  value={item.audit2RecommendedPrice || 0}
                                  onChange={(val) => handleItemChange(index, 'audit2RecommendedPrice', val)}
                                  className="w-24 text-right rounded-md border-stone-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm"
                                  placeholder="Price"
                                />
                                <input
                          ...
                          <>
                            <td className="px-6 py-4 whitespace-nowrap text-center">
                              <input 
                                ...
                                disabled={!canInspect} 
                                onChange={(e) => handleItemChange(index, 'isBought', e.target.checked)} 
                                className="w-4 h-4 rounded border-stone-300 text-orange-600 focus:ring-orange-500 disabled:opacity-50" 
                              />
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-center">
                              <input 
                                type="checkbox" 
                                checked={item.isInspected || false} 
                                disabled={!canInspect} 
                                onChange={(e) => handleItemChange(index, 'isInspected', e.target.checked)} 
                                className="w-4 h-4 rounded border-stone-300 text-orange-600 focus:ring-orange-500 disabled:opacity-50" 
                              />
                            </td>
                          </>
                  ...
                  <tfoot className="bg-stone-50">
                    <tr>
                      <th scope="row" colSpan={
                        3 + 
                        ((isStoreEditing || isAuditEditing || request.items.some(i => i && i.recommendedPrice)) ? 1 : 0) +
                        ((isAuditEditing || request.items.some(i => i && i.audit1RecommendedPrice)) ? 1 : 0) +
                        ((isAuditEditing || request.items.some(i => i && i.audit2RecommendedPrice)) ? 1 : 0) +
                        ((isStoreEditing || isAuditEditing || request.items.some(i => i && i.supplier)) ? 1 : 0) +
                        ((isStoreEditing || isAuditEditing || request.items.some(i => i && i.availability)) ? 1 : 0) +
                        ((isAuditEditing || request.items.some(i => i && (i.flagged || i.aiMarketPrice))) ? 1 : 0)
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-lg font-bold text-orange-600">
                        ₦{(isStoreEditing || isAuditEditing || canInspect ? editableItems : request.items).filter(Boolean).reduce((sum, item) => sum + ((item.quantity || 0) * (item.recommendedPrice || item.estimatedCost || 0)), 0).toLocaleString()}
                      </td>
                    </tr>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right">
                          {isAuditEditing ? (
                            <div className="flex items-center justify-end">
                              ...
                              <MoneyInput
                                value={recommendedAmount === '' ? 0 : recommendedAmount}
                                onChange={(val) => setRecommendedAmount(val)}
          ...
          {request.status === 'Approved' && isInspectionEligible && !hasItems && (
            <div className="bg-white rounded-2xl shadow-sm border border-stone-200 overflow-hidden">
              <div className="p-6 border-b border-stone-100 flex justify-between items-center">
                <h2 className="text-lg font-semibold text-stone-900">Audit Inspection</h2>
                {canInspect && (
                  <button
                    onClick={handleSaveInspection}
                    disabled={isSavingInspection}
              ...
              <div className="p-6 flex gap-8">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input 
                    type="checkbox" 
                    checked={isRequestBought} 
                    onChange={(e) => setIsRequestBought(e.target.checked)} 
                    disabled={!canInspect} 
                    className="w-5 h-5 rounded border-stone-300 text-orange-600 focus:ring-orange-500 disabled:opacity-50" 
                  />
                  <span className="text-sm font-medium text-stone-700">Mark as Bought</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer">
                  <input 
                    type="checkbox" 
                    checked={isRequestInspected} 
                    onChange={(e) => setIsRequestInspected(e.target.checked)} 
                    disabled={!canInspect} 
                    className="w-5 h-5 rounded border-stone-300 text-orange-600 focus:ring-orange-500 disabled:opacity-50" 
                  />
                  <span className="text-sm font-medium text-stone-700">Mark as Inspected</span>
                </label>
              </div>
            </div>
          )}
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-stone-200 overflow-hidden">
              ...
              <div className="p-6">
                <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {request.attachments.filter(Boolean).map((att) => (
                    <li key={att.id} className="flex items-center justify-between p-3 bg-stone-50 border border-stone-200 rounded-xl shadow-sm hover:border-orange-300 transition-colors">
                      <div className="flex items-center space-x-3 truncate">
                        <div className="flex-shrink-0">
                          {(att.type && typeof att.type === 'string' && att.type.toLowerCase().includes('pdf')) || (att.name && typeof att.name === 'string' && att.name.toLowerCase().endsWith('.pdf')) ? (
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-stone-200 overflow-hidden">
              ...
              <div className="p-6">
                <div className="flex items-center justify-between p-3 bg-stone-50 border border-stone-200 rounded-xl shadow-sm hover:border-orange-300 transition-colors max-w-md">
                  <div className="flex items-center space-x-3 truncate">
                    <div className="flex-shrink-0">
                      {(request.paymentInvoice.type && typeof request.paymentInvoice.type === 'string' && request.paymentInvoice.type.toLowerCase().includes('pdf')) || (request.paymentInvoice.name && typeof request.paymentInvoice.name === 'string' && request.paymentInvoice.name.toLowerCase().endsWith('.pdf')) ? (
                        <FileText className="w-6 h-6 text-red-500" />
                      ) : (
                ...
                <ul className="space-y-6">
                  <li className="relative flex gap-4">
                    ...
                    <div className="flex flex-col">
                      ...
                      <p className="text-xs text-stone-500">{safeFormat(request.createdAt, 'MMM d, yyyy h:mm a')}</p>
                    </div>
                  </li>
                            ...
                            {voidRecord && (
                              <div className="mt-2 text-sm text-stone-600 bg-stone-50 p-3 rounded-xl border border-stone-100">
                                <div className="flex justify-between items-center mb-2">
                                  ...
                                  <span className="text-xs text-stone-400">{safeFormat(voidRecord.date, 'MMM d, h:mm a', '')}</span>
                                </div>
                              ...
                              <div className="mt-2 text-sm text-stone-600 bg-stone-50 p-3 rounded-xl border border-stone-100">
                                <div className="flex justify-between items-center mb-2">
                                  ...
                                  <span className="text-xs text-stone-400">
                                    {safeFormat(approvalRecord.date, 'MMM d, h:mm a', '')}
                                    {index === 0 && request.createdAt && approvalRecord.date
                                      ? ` (${calculateDuration(request.createdAt, approvalRecord.date)})`
                                      : request.approvals && request.approvals[index - 1] && request.approvals[index - 1].date && approvalRecord.date
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/80 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1">Void Password</label>
                <input
                  type="password"
                  value={voidPassword}
              ...
              <div className="flex justify-end gap-3 mt-6">
                ...
                <button
                  onClick={handleVoid}
                  disabled={isVoiding}
                  className="px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
                >
                  {isVoiding ? (
                    <>
