      ...
      const similar = requests.filter(r => 
        ...
        r.items?.some(ri => 
          request.items?.some(i => 
            (i.name && ri.name && ri.name.toLowerCase().includes(i.name.toLowerCase())) ||
            (i.description && ri.description && ri.description.toLowerCase().includes(i.description.toLowerCase()))
          )
        )
      );
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-stone-200 overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-stone-200">
                  ...
                  <tbody className="bg-white divide-y divide-stone-200">
                    {(isStoreEditing || isAuditEditing || canInspect ? editableItems : request.items).map((item, index) => (
                  ...
                  <tfoot className="bg-stone-50">
                    <tr>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-lg font-bold text-orange-600">
                        ₦{(isStoreEditing || isAuditEditing || canInspect ? editableItems : request.items).reduce((sum, item) => sum + ((item.quantity || 0) * (item.recommendedPrice || item.estimatedCost || 0)), 0).toLocaleString()}
                      </td>
                    </tr>
