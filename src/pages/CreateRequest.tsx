...
export default function CreateRequest() {
  ...
  const [productProcurementDetails, setProductProcurementDetails] = useState<ProductProcurementDetails>({
    dateOfRequisition: '', issuerName: user?.email || '', department: user?.department || 'Facility', vendorId: ''
  });

  const [leaveDetails, setLeaveDetails] = useState<LeaveDetails>({
    dateOfRequisition: '', issuerName: user?.email || '', applicantName: '', startDate: '', endDate: '', daysRemaining: 0, leaveBalance: 0, purpose: ''
  });
    ...
    try {
      ...
      if (requestType === 'Histology Payment') {
        if (result.details && result.details.length > 0) {
          const newDetails = result.details.map((d: any) => ({
            date: d.date || '',
            patientName: d.patientName || '',
            hospNo: d.hospNo || '',
            labNo: d.labNo || '',
            outsourceService: d.outsourceService || '',
            outsourceBill: d.outsourceBill || 0,
            zmcCharges: d.zmcCharges || 0,
            receiptHmo: d.receiptHmo || '',
            retainership: d.retainership || ''
          }));

          setHistologyDetailsList(prev => {
            // If the first item is empty, replace it. Otherwise append.
            const isFirstEmpty = prev.length === 1 && !prev[0].patientName && !prev[0].hospNo;
            if (isFirstEmpty) return newDetails;
      ...
      addNotification({
        ...
        message: 'Failed to auto-fill form. Please check the document and try again.',
        type: 'error',
      });
    } finally {
  ...
  const handleSaveDraft = async () => {
    if (!title) {
      addNotification({
        userId: user?.id || 'system',
        title: 'Missing Title',
        message: 'Please provide a request title to save as draft.',
        type: 'warning',
      });
      return;
    }

    let finalSignatureUrl = creatorSignature;
    if (creatorSignature && creatorSignature.startsWith('data:')) {
      try {
        finalSignatureUrl = await uploadBase64(creatorSignature, 'signature.png');
      } catch (error) {
    ...
    const requestData = {
      ...
      requestType,
      urgencyLevel,
      creatorSignature: finalSignatureUrl,
    ...
    if (isSavingWorkflow && newWorkflowName) {
      const validApprovers = workflowApprovers.filter(id => id !== '');
    ...
    await saveDraft(requestData, existingRequest?.id);
    navigate('/requests');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title) {
      addNotification({
        userId: user?.id || 'system',
        title: 'Missing Title',
        message: 'Please provide a request title.',
        type: 'warning',
      });
      return;
    }
    if (!creatorSignature) {
      addNotification({
        ...
        message: 'Please sign the request before submitting.',
        type: 'warning',
      });
      return;
    }
    if (workflowApprovers.length === 0) {
      addNotification({
        ...
        message: 'Please select at least one approver for the workflow.',
        type: 'warning',
      });
      return;
    }
    ...
    try {
      ...
      let finalSignatureUrl = creatorSignature;
      if (creatorSignature.startsWith('data:')) {
        try {
          finalSignatureUrl = await uploadBase64(creatorSignature, 'signature.png');
        } catch (error) {
          console.error('Error uploading signature:', error);
        ...
        if (suppliers.length > 1) {
          for (const [idx, supplier] of suppliers.entries()) {
            ...
            if (isEditMode && existingRequest && idx === 0) {
              ...
              const newId = await addRequest(splitData);
              if (splitData.workflow && splitData.workflow.length > 0) {
                addNotification({
                  userId: splitData.workflow[0],
                  title: 'New Request',
      ...
      if (isEditMode && existingRequest) {
        ...
        const newId = await addRequest(requestData);
        if (requestData.workflow && requestData.workflow.length > 0) {
          addNotification({
            userId: requestData.workflow[0],
            title: 'New Request',
          ...
          {(isEquipReq || isPharmPO || isGeneral || isProductProc || isStoreReq || isITProc) && (
            <div>
              ...
              <MoneyInput
                ...
                onChange={(val) => handleItemChange(item.id, 'estimatedCost', val)}
                className="block w-full rounded-lg border-stone-200 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm py-2 px-3 border bg-white"
            ...
            <div className="pt-6 border-t border-stone-100 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input type="number" required min="0" value={dieselDetails.volumeLiters || 0} onChange={e => setDieselDetails({...dieselDetails, volumeLiters: parseFloat(e.target.value) || 0})} className="block w-full rounded-lg border-stone-200 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm py-2 px-3 border" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-stone-500 mb-1">Diesel Remaining (Liters)</label>
                  <input type="number" required min="0" value={dieselDetails.dieselRemainingLiters || 0} onChange={e => setDieselDetails({...dieselDetails, dieselRemainingLiters: parseFloat(e.target.value) || 0})} className="block w-full rounded-lg border-stone-200 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm py-2 px-3 border" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-stone-500 mb-1">Cost per Liter (₦)</label>
                  <MoneyInput required min="0" value={dieselDetails.costPerLiter || 0} onChange={val => setDieselDetails({...dieselDetails, costPerLiter: val})} className="block w-full rounded-lg border-stone-200 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm py-2 px-3 border" />
                </div>
          ...
          {requestType === 'Product Procurement' && (
            <div className="pt-6 border-t border-stone-100 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="md:col-span-3">
                  <label className="block text-xs font-medium text-stone-500 mb-1">Select Vendor</label>
                  <select required value={productProcurementDetails.vendorId || ''} onChange={e => setProductProcurementDetails({...productProcurementDetails, vendorId: e.target.value})} className="block w-full rounded-lg border-stone-200 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm py-2 px-3 border">
                    <option value="">-- Select a Vendor --</option>
                    {vendors.map(v => (
                      <option key={v.id} value={v.id}>{v.name}</option>
                    ))}
                ...
                {productProcurementDetails.vendorId === 'manual' && (
                  <div className="md:col-span-3">
                    <label className="block text-xs font-medium text-stone-500 mb-1">Vendor Name (Manual)</label>
                    <input type="text" required value={productProcurementDetails.manualVendorName || ''} onChange={e => setProductProcurementDetails({...productProcurementDetails, manualVendorName: e.target.value})} className="block w-full rounded-lg border-stone-200 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm py-2 px-3 border" />
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="pt-6 border-t border-stone-100 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select required value={leaveDetails.purpose || ''} onChange={e => setLeaveDetails({...leaveDetails, purpose: e.target.value})} className="block w-full rounded-lg border-stone-200 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm py-2 px-3 border bg-white">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                ...
                {requestType === 'Issue From Store' && (
                  <div>
                    ...
                    <select 
                    required 
                    value={issueFromStoreDetails.departmentToIssueTo || ''} 
                    onChange={e => setIssueFromStoreDetails({...issueFromStoreDetails, departmentToIssueTo: e.target.value as Department})} 
                    className="block w-full rounded-lg border-stone-200 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm py-2 px-3 border"
                  >
                      <option value="Facility">Facility</option>
                      <option value="Laboratory">Laboratory</option>
              ...
              <div className="overflow-x-auto -mx-4 sm:mx-0">
                <table className="min-w-full divide-y divide-stone-200">
                  <thead className="bg-stone-50">
                    <tr>
                      <th className="px-3 py-3 text-left text-xs font-medium text-stone-500 uppercase tracking-wider">Date</th>
                      <th className="px-3 py-3 text-left text-xs font-medium text-stone-500 uppercase tracking-wider">Patient Name</th>
                      <th className="px-3 py-3 text-left text-xs font-medium text-stone-500 uppercase tracking-wider">Hosp No.</th>
                      <th className="px-3 py-3 text-left text-xs font-medium text-stone-500 uppercase tracking-wider">Lab No.</th>
                      <th className="px-3 py-3 text-left text-xs font-medium text-stone-500 uppercase tracking-wider">Outsource Service</th>
                      <th className="px-3 py-3 text-right text-xs font-medium text-stone-500 uppercase tracking-wider">Outsource Bill</th>
                      <th className="px-3 py-3 text-right text-xs font-medium text-stone-500 uppercase tracking-wider">ZMC Charges</th>
                      <th className="px-3 py-3 text-left text-xs font-medium text-stone-500 uppercase tracking-wider">Receipt/HMO</th>
                      <th className="px-3 py-3 text-left text-xs font-medium text-stone-500 uppercase tracking-wider">Retainership</th>
                      <th className="px-3 py-3 text-center text-xs font-medium text-stone-500 uppercase tracking-wider">Action</th>
                    </tr>
                  </thead>
                    ...
                    {histologyDetailsList.map((detail, index) => (
                      <tr key={index} className="hover:bg-stone-50 transition-colors">
                        <td className="px-2 py-3">
                          <input type="date" required value={detail.date || ''} onChange={e => { const newList = [...histologyDetailsList]; newList[index].date = e.target.value; setHistologyDetailsList(newList); }} className="block w-full rounded-lg border-stone-200 shadow-sm focus:border-orange-500 focus:ring-orange-500 text-xs py-1.5 px-2 border" />
                        </td>
                        <td className="px-2 py-3">
                          <input type="text" required value={detail.patientName || ''} onChange={e => { const newList = [...histologyDetailsList]; newList[index].patientName = e.target.value; setHistologyDetailsList(newList); }} className="block w-32 rounded-lg border-stone-200 shadow-sm focus:border-orange-500 focus:ring-orange-500 text-xs py-1.5 px-2 border" placeholder="Name" />
                        </td>
                        <td className="px-2 py-3">
                          <input type="text" required value={detail.hospNo || ''} onChange={e => { const newList = [...histologyDetailsList]; newList[index].hospNo = e.target.value; setHistologyDetailsList(newList); }} className="block w-24 rounded-lg border-stone-200 shadow-sm focus:border-orange-500 focus:ring-orange-500 text-xs py-1.5 px-2 border" placeholder="Hosp No" />
                        </td>
                        <td className="px-2 py-3">
                          <input type="text" required value={detail.labNo || ''} onChange={e => { const newList = [...histologyDetailsList]; newList[index].labNo = e.target.value; setHistologyDetailsList(newList); }} className="block w-24 rounded-lg border-stone-200 shadow-sm focus:border-orange-500 focus:ring-orange-500 text-xs py-1.5 px-2 border" placeholder="Lab No" />
                        </td>
                        <td className="px-2 py-3">
                          <input type="text" required value={detail.outsourceService || ''} onChange={e => { const newList = [...histologyDetailsList]; newList[index].outsourceService = e.target.value; setHistologyDetailsList(newList); }} className="block w-32 rounded-lg border-stone-200 shadow-sm focus:border-orange-500 focus:ring-orange-500 text-xs py-1.5 px-2 border" placeholder="Service" />
                        </td>
                        <td className="px-2 py-3">
                          <MoneyInput required min="0" value={detail.outsourceBill || 0} onChange={val => { const newList = [...histologyDetailsList]; newList[index].outsourceBill = val; setHistologyDetailsList(newList); }} className="block w-24 rounded-lg border-stone-200 shadow-sm focus:border-orange-500 focus:ring-orange-500 text-xs py-1.5 px-2 border text-right" />
                        </td>
                        <td className="px-2 py-3">
                          <MoneyInput required min="0" value={detail.zmcCharges || 0} onChange={val => { const newList = [...histologyDetailsList]; newList[index].zmcCharges = val; setHistologyDetailsList(newList); }} className="block w-24 rounded-lg border-stone-200 shadow-sm focus:border-orange-500 focus:ring-orange-500 text-xs py-1.5 px-2 border text-right" />
                        </td>
                        <td className="px-2 py-3">
                          <input type="text" required value={detail.receiptHmo || ''} onChange={e => { const newList = [...histologyDetailsList]; newList[index].receiptHmo = e.target.value; setHistologyDetailsList(newList); }} className="block w-24 rounded-lg border-stone-200 shadow-sm focus:border-orange-500 focus:ring-orange-500 text-xs py-1.5 px-2 border" placeholder="Receipt" />
                        </td>
                        <td className="px-2 py-3">
                          <input type="text" required value={detail.retainership || ''} onChange={e => { const newList = [...histologyDetailsList]; newList[index].retainership = e.target.value; setHistologyDetailsList(newList); }} className="block w-24 rounded-lg border-stone-200 shadow-sm focus:border-orange-500 focus:ring-orange-500 text-xs py-1.5 px-2 border" placeholder="Retainership" />
                        </td>
          ...
          {requestType.startsWith('Emergency Drug Purchase') && (
            <div className="pt-6 border-t border-stone-100 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div><label className="block text-xs font-medium text-stone-500 mb-1">The Sum Of (Amount ₦)</label><MoneyInput required min="0" value={paymentDetails.amount || 0} onChange={val => setPaymentDetails({...paymentDetails, amount: val})} className="block w-full rounded-lg border-stone-200 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm py-2 px-3 border" /></div>
            ...
            {!selectedWorkflowId && workflowApprovers.length > 0 && (
              <div className="mt-4 p-4 bg-stone-50 rounded-xl border border-stone-200">
                <label className="flex items-center gap-2 text-sm text-stone-700 mb-2">
                  <input
                    type="checkbox"
                    checked={isSavingWorkflow}
                    onChange={(e) => setIsSavingWorkflow(e.target.checked)}
                ...
                {isSavingWorkflow && (
                  <input
                    type="text"
                    placeholder="Workflow Name (e.g., Standard Lab Approval)"
          ...
          <div className="pt-6 border-t border-stone-100 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex gap-3 w-full sm:w-auto">
              ...
              <button
                ...
                className="flex-1 sm:flex-none inline-flex justify-center items-center px-6 py-2 border border-transparent text-sm font-medium rounded-xl shadow-sm text-white bg-orange-600 hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition-colors disabled:opacity-50"
              >
                <Save className="w-4 h-4 mr-2" />
                {isSubmitting ? 'Submitting...' : (isEditMode && existingRequest?.status !== 'Draft' ? 'Resubmit Request' : 'Submit Request')}
              </button>
            </div>
          </div>
