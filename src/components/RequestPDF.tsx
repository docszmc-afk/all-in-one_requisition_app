      ...
      <Page size="A4" style={styles.page}>
        ...
        {isHistology && (request.histologyDetailsList || request.histologyDetails) && (
          <View style={styles.section}>
            ...
            <View style={styles.table}>
              ...
              {(request.histologyDetailsList || (request.histologyDetails ? [request.histologyDetails] : [])).map((detail, index) => (
          ...
          <View style={styles.section}>
            ...
            <View style={styles.row}>
              {request.leaveDetails.leaveBalance !== undefined && (
                <View style={styles.col}><Text style={styles.label}>Leave Days Left</Text><Text style={styles.value}>{request.leaveDetails.leaveBalance}</Text></View>
              )}
              <View style={styles.col}><Text style={styles.label}>Purpose</Text><Text style={styles.value}>{request.leaveDetails.purpose}</Text></View>
            </View>
          </View>
        )}
              ...
              {request.recommendedAmount !== undefined && (
                <View style={[styles.totalRow, { backgroundColor: '#fff7ed', borderTopWidth: 1, borderTopColor: '#fdba74' }]}>
                  ...
                  <Text style={[styles.totalValue, { color: '#c2410c', fontSize: 14 }]}>
                    N{(request.recommendedAmount || 0).toLocaleString()}
                  </Text>
                </View>
              )}
            </View>
        ...
        <View style={styles.workflowSection}>
          ...
          {request.workflow && request.workflow.length > 0 ? (
            request.workflow
              .map((approverId, index) => ({ approverId, index }))
              .filter(({ approverId, index }) => {
                ...
                return approvalRecord || voidRecord;
              })
